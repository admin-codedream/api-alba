...
| `INSURANCE_RATE_RULES` | 4대보험 요율 (기준일자별) |
| `EMPLOYEE_INSURANCE_SETTINGS` | 직원별 4대보험 적용 설정 |
...
### 4대보험 자동공제 DDL
```sql
CREATE TABLE INSURANCE_RATE_RULES
(
    ID             BIGINT UNSIGNED AUTO_INCREMENT COMMENT '보험 요율 PK' PRIMARY KEY,
    INSURANCE_TYPE VARCHAR(30)    NOT NULL COMMENT '보험 유형(NATIONAL_PENSION, HEALTH_INSURANCE, LONG_TERM_CARE, EMPLOYMENT_INSURANCE)',
    RATE_TARGET    VARCHAR(30)    NOT NULL COMMENT '부담 대상(EMPLOYEE, EMPLOYER, TOTAL)',
    RATE           DECIMAL(8, 5)  NOT NULL COMMENT '요율 (예: 0.04750)',
    BASE_TYPE      VARCHAR(30)    NOT NULL COMMENT '계산 기준(TAXABLE_WAGE: 과세급여, HEALTH_INSURANCE_AMOUNT: 건강보험 공제액)',
    EFFECTIVE_FROM DATE           NOT NULL COMMENT '적용 시작일',
    EFFECTIVE_TO   DATE           NULL COMMENT '적용 종료일 (null이면 현재까지 유효)',
    ...
    INDEX IDX_INSURANCE_RATE_RULES_TYPE_DATE (INSURANCE_TYPE, RATE_TARGET, EFFECTIVE_FROM)
) COMMENT '4대보험 요율 기준일자별 관리' CHARSET = utf8mb4;

-- 2026년 기준 초기 데이터 (귀속월 말일 기준으로 조회)
INSERT INTO INSURANCE_RATE_RULES (INSURANCE_TYPE, RATE_TARGET, RATE, BASE_TYPE, EFFECTIVE_FROM, EFFECTIVE_TO) VALUES
('NATIONAL_PENSION',    'EMPLOYEE', 0.04500, 'TAXABLE_WAGE',            '2023-01-01', '2025-12-31'),
('NATIONAL_PENSION',    'EMPLOYEE', 0.04500, 'TAXABLE_WAGE',            '2026-01-01', NULL),
('HEALTH_INSURANCE',    'EMPLOYEE', 0.03545, 'TAXABLE_WAGE',            '2024-01-01', '2025-12-31'),
('HEALTH_INSURANCE',    'EMPLOYEE', 0.03595, 'TAXABLE_WAGE',            '2026-01-01', NULL),
('LONG_TERM_CARE',      'EMPLOYEE', 0.12810, 'HEALTH_INSURANCE_AMOUNT', '2024-01-01', '2025-12-31'),
('LONG_TERM_CARE',      'EMPLOYEE', 0.13140, 'HEALTH_INSURANCE_AMOUNT', '2026-01-01', NULL),
('EMPLOYMENT_INSURANCE','EMPLOYEE', 0.00900, 'TAXABLE_WAGE',            '2022-07-01', NULL);

CREATE TABLE EMPLOYEE_INSURANCE_SETTINGS
(
    ID                     BIGINT UNSIGNED AUTO_INCREMENT COMMENT '직원 보험 설정 PK' PRIMARY KEY,
    WORKPLACE_MEMBER_ID    BIGINT UNSIGNED NOT NULL COMMENT '사업장 멤버 ID (WORKPLACE_MEMBERS.ID)',
    USE_NATIONAL_PENSION   TINYINT(1) DEFAULT 0 NOT NULL COMMENT '국민연금 적용 여부',
    USE_HEALTH_INSURANCE   TINYINT(1) DEFAULT 0 NOT NULL COMMENT '건강보험 적용 여부',
    USE_LONG_TERM_CARE     TINYINT(1) DEFAULT 0 NOT NULL COMMENT '장기요양보험 적용 여부',
    USE_EMPLOYMENT_INSURANCE TINYINT(1) DEFAULT 0 NOT NULL COMMENT '고용보험 적용 여부',
    USE_INCOME_TAX         TINYINT(1) DEFAULT 0 NOT NULL COMMENT '소득세 적용 여부',
    TAX_FREE_AMOUNT        DECIMAL(12, 2) DEFAULT 0.00 NOT NULL COMMENT '비과세 금액',
    CREATED_AT             TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT '생성일시',
    UPDATED_AT             TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시',
    UNIQUE KEY UK_EMPLOYEE_INSURANCE_SETTINGS_MEMBER (WORKPLACE_MEMBER_ID)
) COMMENT '직원별 4대보험 적용 설정' CHARSET = utf8mb4;

-- PAYSLIP_DEDUCTIONS 컬럼 추가 (자동 계산 적용 요율/기준액 기록용)
ALTER TABLE PAYSLIP_DEDUCTIONS
    ADD COLUMN APPLIED_RATE        DECIMAL(8, 5) NULL COMMENT '자동 계산 시 적용 요율',
    ADD COLUMN APPLIED_BASE_AMOUNT DECIMAL(12, 2) NULL COMMENT '자동 계산 시 계산 기준 금액';
```
...
ALTER TABLE WORKPLACES
    ADD COLUMN QR_PIN VARCHAR(20) NULL COMMENT 'QR 화면 잠금 PIN (null이면 미설정)';
```
...
| 2026-06-12 | `WORKPLACES.QR_PIN` 추가 (QR 화면 잠금 PIN) |
| 2026-06-12 | `INSURANCE_RATE_RULES` 테이블 추가 (4대보험 요율 기준일자 관리) |
| 2026-06-12 | `EMPLOYEE_INSURANCE_SETTINGS` 테이블 추가 (직원별 4대보험 적용 설정) |
| 2026-06-12 | `PAYSLIP_DEDUCTIONS.APPLIED_RATE`, `PAYSLIP_DEDUCTIONS.APPLIED_BASE_AMOUNT` 추가 (자동 계산 감사 추적용) |
